...
namespace Jtar\Elasticsearch\Index;
...
//    const TYPE_ARRAY = 'array'; // 在同一个数组中，数组元素的数据类型是相同的，ElasticSearch不支持元素为多个数据类型，7.0版本已废弃
    ...
    {
        $this->client = $client ?: make(BuilderFactory::class)->getClient();
    }
    ...
    {
        if (isset($this->fields[static::class])) {
            return $this->fields[static::class];
        }

        return $this->fields[static::class] = array_keys($this->mapping['properties'] ?? []);
    }

    public function make(...$params): ElasticsearchIndex
    {
